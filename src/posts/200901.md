...
# How
그럼 CSS Houdini를 직접 써보자. 준비물은 Chomium browser와 간단한 코드 에디터이다.
...
이는 background image가 element를 넘어서 그려질 수 없기 때문이다. 그렇기에 약간의 편법을 활용해야 하는데,
...
```
이렇게 하면 아래처럼 element가 생성된다. 썩 마음에 들지 않는 코드일지는 몰라도, *어쨌든* 그림자를 보여줄 수 있다.
...
다만 주의할 점은, 아직 모든 API 가 전부 구현된 것은 아니다. MDN 문서도 부실한 부분이 있으며, 브라우저 별로도 지원 현황이 다르다. [여기](https://ishoudinireadyyet.com)에서 브라우저 별 지원 현황을 확인할 수 있다. paint API 를 위한 polyfill 이 존재하기는 하지만, 어쨌든 지금 production 에서 공격적으로 사용하기에는 시기상조라는 생각이 든다.
